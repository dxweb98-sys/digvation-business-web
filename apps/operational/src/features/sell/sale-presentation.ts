...
export function saleLineDiscountPercentage(
  sale: Pick<Sale, 'adjustments'>,
  line: Pick<SaleLine, 'id' | 'discountType' | 'discountValue'>,
): string | null {
  const lineAdjustments = (sale.adjustments ?? []).filter(
    (adjustment) => adjustment.saleLineId === line.id && positive(adjustment.actualAmount),
  );
  if (lineAdjustments.length > 1) return null;
  const adjustment = lineAdjustments[0];
  if (adjustment)
    return adjustment.type === 'PERCENTAGE'
      ? percentageFromRate(adjustment.configuredValue)
      : null;
  return lineDiscountPercentage(line);
}
