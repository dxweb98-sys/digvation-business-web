...
const localizedActivityCopy: Record<string, { id: string; en: string }> = {
  ...
  'Transaction item quantity changed': { id: 'Jumlah item transaksi diubah', en: 'Transaction item quantity changed' },
  'Transaction item removed': { id: 'Item transaksi dihapus', en: 'Transaction item removed' },
  'Transaction item price overridden': { id: 'Harga item transaksi dioverride', en: 'Transaction item price overridden' },
  'Transaction item price override removed': { id: 'Override harga item transaksi dihapus', en: 'Transaction item price override removed' },
  'Transaction item discount applied': { id: 'Diskon item transaksi diterapkan', en: 'Transaction item discount applied' },
  'Transaction item discount removed': { id: 'Diskon item transaksi dihapus', en: 'Transaction item discount removed' },
  'Transaction discount applied': { id: 'Diskon transaksi diterapkan', en: 'Transaction discount applied' },
  'Transaction discount removed': { id: 'Diskon transaksi dihapus', en: 'Transaction discount removed' },
  'Transaction item assignment changed': { id: 'Penugasan item transaksi diubah', en: 'Transaction item assignment changed' },
  'Transaction item contribution changed': { id: 'Kontribusi item transaksi diubah', en: 'Transaction item contribution changed' },
...
export function ActivityPage() {
  ...
  const columns: TableColumn<ActivityEvent>[] = [
    ...
    { key: 'eventType', label: copy('Action'), render: (item) => eventLabel(item.eventType, activityCopy) },
    { key: 'target', label: copy('Target / reference'), render: (item) => targetSummary(item, activityCopy) ?? '—' },
    {
      key: 'category',
      label: copy('Category'),
      render: (item) => <DBadge variant="outline">{activityCopy(categoryLabels[item.category] ?? item.category)}</DBadge>,
    },
        ...
        <DDataTable
          ...
          filters={
            <div className="flex flex-wrap gap-2">
              ...
              <DSelectFilter label={copy('Category')} value={category || null} clearable options={Object.entries(categoryLabels).map(([value, label]) => ({ value, label: activityCopy(label) }))} onChange={(value) => { setCategory(String(value ?? '')); resetPage(); }} />
