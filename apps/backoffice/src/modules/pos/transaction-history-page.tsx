...
export function TransactionHistoryPage() {
  ...
  const columns: TableColumn<Sale>[] = [
    ...
    { key: 'payments', label: copy('Payments'), render: (row) => row.payments.length ? <div className="flex flex-wrap gap-1">{row.payments.map((payment) => <StatusBadge key={payment.id} status={payment.status} />)}</div> : 'â€”' },
